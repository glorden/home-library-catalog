...
| `AI_PROXY_URL` | 9 | `socks5h://ai-proxy:1080` — включает проксирование вызовов Claude/Gemini через сайдкар-туннель; пусто, если не нужно |
| `AI_PROXY_SSH_HOST` | 9 | адрес внешнего VPS, на который туннелирует сайдкар `ai-proxy` |
| `AI_PROXY_SSH_USER` | 9 | пользователь на внешнем VPS для туннеля (по умолчанию `tunneluser`) |
...
## Шаг 9: AI-провайдер из заблокированной страны (SOCKS5-туннель)

Прод-VPS геолоцирован в России — Anthropic, OpenAI и Google исключают
Россию из supported-countries для своих API (экспортные ограничения США,
не особенность конкретного вендора — проверено официальными списками всех
трёх). Реальный вызов упал `400 FAILED_PRECONDITION: "User location is not
supported for the API use."` — не баг конфига, гео-блок на стороне
вендора; смена AI-провайдера на этом же сервере не помогает.

Решение — не переносить весь VPS, а завернуть только исходящие вызовы
`claude_provider.py`/`openai_compatible_provider.py` в SOCKS5-туннель через
SSH на сервер в незаблокированной стране. SSH выбран вместо Squid/HTTP-proxy
на внешнем сервере: не добавляет новый открытый порт и новый тип секрета —
переиспользует ту же модель доверия по ключам, что уже принята для
`deploy`-пользователя основного VPS.

### Устройство

Сайдкар `ai-proxy` (`docker/ai-proxy/`, только в `docker-compose.prod.yml`,
не в dev) держит один процесс `ssh -N -D 0.0.0.0:1080` наружу, на
`AI_PROXY_SSH_HOST`. Порт 1080 никуда не публикуется (`ports:` нет) — виден
только сиблинг-контейнерам по compose-сети, то есть без аутентификации на
уровне SOCKS (не нужна — тот же уровень доверия, что уже принят для `db`).
`registry.py` — единственная точка, где `httpx.Client(proxy=AI_PROXY_URL)`
собирается и передаётся в оба провайдера через `http_client=`; при пустом
`AI_PROXY_URL` поведение не меняется (прямые вызовы, как раньше).

### Внешний сервер: минимальные требования, не полная настройка с нуля

В отличие от основного VPS, внешний сервер под туннель может быть уже
занят под другую задачу (в нашем случае — личный Xray/x-ui пользователя).
Поэтому здесь **не повторяется** весь чек-лист «VPS: базовая настройка»
выше — вместо этого:

- Создать пользователя без шелла (`useradd -m -s /usr/sbin/nologin
  tunneluser`) — не трогает существующих пользователей/процессы/порты на
  этом сервере.
- Пара ключей генерируется **на проде** (`ssh-keygen` прямо на VPS, не на
  машине разработчика) и живёт в `secrets/ai_proxy_id_ed25519` (гитигнор,
...
- **`sshd_config` внешнего сервера не трогается** — если на нём уже был
  включён, например, `PasswordAuthentication yes` для существующего
  доступа владельца, эта настройка остаётся как есть. Хардening внешнего
  сервера — не наша ответственность, если сервер уже используется для
  чего-то ещё.
- `known_hosts` фиксируется один раз через `ssh-keyscan` (с прод-VPS, при
...
Прокси включается глобально (`registry.py`), не по credential — если
когда-нибудь понадобится одновременно локальная модель (Ollama/LM Studio,
для чего и задуман `openai_compatible`) и включённый `AI_PROXY_URL` для
Gemini/Claude, локальный трафик тоже уйдёт во внешний туннель и сломается.
Отдельного тумблера на credential пока нет — добавить при реальной
необходимости.
...
```bash
docker compose -f docker-compose.yml -f docker-compose.prod.yml exec app \
  python -c "import httpx; print(httpx.get('https://ipapi.co/json/', proxy='socks5h://ai-proxy:1080', timeout=15).json())"
```
Страна в ответе должна быть страной внешнего сервера, не прод-VPS. Дальше —
реальный вызов `/admin/extract/recognize` с настоящим фото, без
`FAILED_PRECONDITION` в ответе.
