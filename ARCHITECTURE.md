...
- [x] Шаг 9 — AI-провайдер из заблокированной страны (подробности —
      ...
      **Неожиданный результат**: сам туннель не решил проблему с Gemini —
      ...
      Реальный вызов с прода через настоящую сессию владельца подтверждён в
      `extraction_calls` (`success=true`, `qwen/qwen3.6-27b`,
      `image_count=1` — только обложка) — не только на уровне сервиса, но и
      через настоящий `/admin/extract/recognize`.

      Отдельная находка после первого успеха: подняли `EXTRACTION_MAX_TOKENS`
      с 4096 до 6000, рассчитывая на переменную длину "рассуждения" модели
      перед вызовом функции (см. `app/services/extraction/base.py`) — и
      сразу словили 100%-й отказ `413 rate_limit_exceeded` ("Requested
      9847" при лимите 8000 TPM). У Groq квота считается по *запрошенному*
      `max_tokens`, а не по фактически использованным токенам — ещё до
      генерации. Откатили обратно на 4096: это существующий компромисс
      (модель иногда всё равно обрывается на сложном фото с
      `tool_use_failed`), но подниматься с ним для этой модели/тарифа
      больше нельзя, не упёршись в первую ошибку раньше второй.

      Заодно по итогам живого использования: (1) `logger.info(...)` в
      обоих провайдерах раньше нигде не появлялись — ни в проде, ни в деве
      — потому что в проекте не было ни одной настройки `logging`, а
      root-логгер без хендлеров молча роняет INFO ниже своего
      `lastResort` (WARNING). `app/main.py`, `_configure_logging()` вешает
      хендлер именно на неймспейс `"app"` (не на root) — сторонние
      библиотеки (httpx, openai) не задеты, проверено вживую отдельным
      DEBUG-сообщением, что оно не пролезло. (2) Кнопка «Распознать» в
      `extract_upload.html` теперь дизейблится и меняет текст на submit —
      без этого не было визуальной обратной связи, что запрос вообще
      ушёл. Подловил себя на реальном багe при проверке в браузере:
      `document.querySelector("form")` брал не ту форму — в `base.html`
      раньше по DOM идёт форма выхода из аккаунта, у неё тоже
      `button[type=submit]` — почему у формы распознавания теперь
      `id="recognize-form"` и `getElementById`, а не общий `querySelector`.
