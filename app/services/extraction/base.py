...
# Было 4096 — на реальном фото у Groq/qwen3.6-27b модель перед вызовом
# функции делает "рассуждение" переменной длины (проверено вживую: 336-1791
# токенов на одном и том же простом изображении) и на сложном фото иногда не
# укладывалась, обрываясь с `tool_use_failed` вместо структурированного
# ответа. 6000, а не больше — у qwen3.6-27b лимит 8000 TPM, вход уже ~2700
# токенов, поднимать выше означало бы упираться в 429 вместо этой ошибки.
